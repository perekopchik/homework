...
button.addEventListener('click', async function () {
    ...
    document.querySelector('.showBody').addEventListener('click', async function () {
        const body1 = document.querySelector('.body');
        const body = document.querySelector('.js--body');
        const email = document.querySelector('.js--email');
        const name = document.querySelector('.js--name');
        body1.classList.add('hidden');
        const response = (await fetch(`https://jsonplaceholder.typicode.com/comments/${user.id}`)).json()
        const userComment = await response;
